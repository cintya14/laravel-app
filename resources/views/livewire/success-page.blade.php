          ...
          <div class="flex items-center justify-center w-full pb-6 space-x-4 md:justify-start">
            <div class="flex flex-col items-start justify-start space-y-2">
              <p class="text-lg font-semibold leading-4 text-left text-gray-800 dark:text-gray-400">
               {{ $order->direccion->nombre }} {{ $order->direccion->apellido }} </p>
              <p class="text-sm leading-4 text-gray-600 dark:text-gray-400"> {{$order->direccion->direccion_calle}}</p>
              <p class="text-sm leading-4 text-gray-600 dark:text-gray-400"> {{$order->direccion->ciudad}}, {{$order->direccion->pais}} {{$order->direccion->codigo_postal}}  </p>
              <p class="text-sm leading-4 cursor-pointer dark:text-gray-400"> {{$order->direccion->telefono}} </p>
            </div>
          </div>
        </div>
      </div>
      <div class="flex flex-wrap items-center pb-4 mb-10 border-b border-gray-200 dark:border-gray-700">
        ...
        <div class="w-full px-4 mb-4 md:w-1/4">
          ...
          <p class="text-base font-semibold leading-4 text-blue-600 dark:text-gray-400">
            {{Number::currency($order->total_general, 'PEN')}} </p>
        </div>
        <div class="w-full px-4 mb-4 md:w-1/4">
          <p class="mb-2 text-sm leading-5 text-gray-600 dark:text-gray-400 ">
            Metodo de pago: </p>
          <p class="text-base font-semibold leading-4 text-gray-800 dark:text-gray-400 ">
            ...
            {{$order->metodo_pago == 'cod' ?  'Pago contra entrega':'Tarjeta de credito'}} </p>
        </div>
      </div>
          ...
          <div class="flex flex-col w-full space-y-6 ">
            ...
            <div class="flex flex-col items-center justify-center w-full pb-4 space-y-4 border-b border-gray-200 dark:border-gray-700">
              <div class="flex justify-between w-full">
                ...
                <p class="text-base leading-4 text-gray-600 dark:text-gray-400">{{Number::currency($order->total_general, 'PEN')}} </p>
              </div>
            ...
            <div class="flex items-center justify-between w-full">
              ...
              <p class="text-base font-semibold leading-4 text-gray-600 dark:text-gray-400">{{Number::currency($order->total_general, 'PEN')}} </p>
            </div>
          </div>
              ...
              <div class="flex items-center justify-center space-x-2">
                ...
                <div class="flex flex-col items-center justify-start">
                  <p class="text-lg font-semibold leading-6 text-gray-800 dark:text-gray-400">
                    Delivery<br><span class="text-sm font-normal">Delivery por 24 Horas</span>
                  </p>
                </div>
              </div>
              <p class="text-lg font-semibold leading-6 text-gray-800 dark:text-gray-400"> 
